...
- AMS (agentic memory system) adapters implementing the shared contract in `packages/coding-agent/scripts/memswe-adapter-contract.ts` (reset/seed/run lifecycle): `hindsight` (first real target, see `SOUL.md`), `graphiti`, `letta`, `mem0`, `supermemory`, and `localrag` (BM25 baseline). Each smoke script skips (not fails) when its service env vars are unset — see per-adapter `*_API_URL`/`*_API_KEY` conventions in `packages/coding-agent/scripts/memswe-adapter-*.ts`.
