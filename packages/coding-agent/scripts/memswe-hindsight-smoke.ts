...
async function loadTaskFacts(): Promise<FactSpec[]> {
	...
	const gradedSessionId = (sessions.find((session) => session.graded) ?? sessions.at(-1))?.session_id;
	if (!gradedSessionId) throw new Error(`Task ${TASK_ID} has no graded or fallback session`);
	return validFactsBeforeSession(task, gradedSessionId);
}
